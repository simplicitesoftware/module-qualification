...
public class QualExUsr extends ObjectDB {
    ...
    public void postLoad() {
        ...
        if (isCandidate(g)) {
            String searchSpec = "t.row_id in (select row_id from qual_ex_usr ans join qual_user_exam ex on ans.QUAL_EXUSR_USREXAM_ID = ex.row_id where QUAL_USREXAM_USR_ID = '"
                    + g.getUserId() + "')";
    ...
    public void initUpdate() {
        ...
        String sqlTotal = "select count(*) from qual_ex_usr where qual_exusr_usrexam_id = '"
                + getFieldValue("qualExusrUsrexamId") + "'";
        String sqlCurrent = "select count(*) from qual_ex_usr where qual_exusr_usrexam_id = '"
                + getFieldValue("qualExusrUsrexamId") + "' and qual_exusr_submitted = '1'";
    ...
    private String displayNext(String rowId) {
        ...
        String nextRow = g.simpleQuery("select row_id from qual_ex_usr where qual_exusr_usrexam_id = '" + userExamRowId
                + "' and row_id > " + rowId + " and QUAL_EXUSR_SUBMITTED = '0' limit 1");
        String prevRow = g.simpleQuery("select row_id from qual_ex_usr where qual_exusr_usrexam_id = '" + userExamRowId
                + "' and row_id < " + rowId + " and QUAL_EXUSR_SUBMITTED = '0' limit 1");
        if ("".equals(nextRow) && "".equals(prevRow)) {
        ...
        } else if (!"".equals(nextRow)) {
            rowId = nextRow;
        } else if (!"".equals(prevRow)) {
            rowId = prevRow;
        }
